...
<simconf>
  ...
  <simulation>
    ...
    <motetype>
      org.contikios.cooja.mspmote.Exp5438MoteType
      <identifier>exp5438#mdc</identifier>
      <description>MDC Silent Collector</description>
      <source EXPORT="discard">[CONTIKI_DIR]/examples/tsch/rpl-udp/border-router-collector/border-router-collector.c</source>
      <commands EXPORT="discard">make border-router-collector.exp5438 TARGET=exp5438</commands>
      <firmware EXPORT="copy">[CONTIKI_DIR]/examples/tsch/rpl-udp/border-router-collector/border-router-collector.exp5438</firmware>
      <moteinterface>org.contikios.cooja.interfaces.Position</moteinterface>
      <moteinterface>org.contikios.cooja.interfaces.RimeAddress</moteinterface>
      <moteinterface>org.contikios.cooja.interfaces.IPAddress</moteinterface>
      <moteinterface>org.contikios.cooja.interfaces.Mote2MoteRelations</moteinterface>
    ...
    <mote>
      ...
      <motetype_identifier>exp5438#mdc</motetype_identifier>
    </mote>
  </simulation>
  <plugin>
  ...
  <plugin>
    Mobility
    <plugin_config>
      <positions EXPORT="copy">[APPS_DIR]/mobility/mdc_random40.dat</positions>
    </plugin_config>
    <width>500</width>
    <z>4</z>
    <height>150</height>
  ...
  <plugin>
    ...
    <plugin_config>
      <script>// IDAP 2026 - Proposed (Gossip + MDC) Experiment (Cooja ScriptRunner)
// Reads env: RESULTS_DIR (default "."), DEADLINE_US (default 3600000000),
//            MDC_ID (default 42) — node id of the mobile collector.
// Files under RESULTS_DIR: raw.log, packets_tx.csv, packets_rx.csv,
// energy.csv, tsch_join.csv, rpl_events.csv, mac_tx.csv,
// parent_change.csv, sixp_events.csv, gossip_events.csv,
// cache_snapshots.csv, mdc_events.csv, summary.txt.
...
var MDC_ID      = parseInt(envOr("MDC_ID", "42"), 10);
...
writeHeader("gossip_events.csv", "sim_time_us,node_id,event,detail");
writeHeader("cache_snapshots.csv","sim_time_us,observer_id,known_count,entries");
writeHeader("mdc_events.csv",    "sim_time_us,event,detail");
...
// Gossip layer aggregates
var gossipTx = {};      // per-node gossip TX count
var gossipRx = {};      // per-node gossip RX count
var cacheOf  = {};      // cacheOf[observer_id] = { target_id: seq } — updated on ADHOC_SYNC lines
var latestSeq= {};      // latestSeq[target_id] = highest seq seen anywhere
for (var n = 1; n &lt;= 60; n++) { gossipTx[n]=0; gossipRx[n]=0; cacheOf[n]={}; }

// MDC-specific
var mdcCache      = {};       // MDC's cache view {target: seq}
var mdcContactRx  = 0;        // count of gossip frames received by MDC
var mdcFirstRxUs  = -1;       // sim time of MDC's first gossip reception (i.e., first contact)
var mdcJoinedTsch = false;    // has MDC synced to TSCH?
...
log.log("\n=========================================\n");
log.log("IDAP 2026 - Proposed (Gossip + MDC)\n");
log.log("Total motes    : " + nodeCount + " (1 root + " + (nodeCount - 2) + " clients + 1 MDC)\n");
log.log("MDC id         : " + MDC_ID + "\n");
...
// RX regex: base emits "... latency N ms", proposed emits "... (Gossip UDP Table)".
// Match the common prefix; group 3 (reported latency) is optional.
...
var reAdhocTx    = /ADHOC TX \[/;
var reAdhocRx    = /Received adhoc frame, hdr=/;
var reAdhocSync  = /ADHOC_SYNC: (\d+)(.*)/;
var reAdhocEntry = /\[(\d+):(\d+)\]/g;
var reMdcStatus  = /MDC status tick=(\d+)/;
var reMdcBoot    = /MDC boot: silent collector/;
...
function finish() {
    ...
    // Gossip + MDC extras
    var gTxTot = 0, gRxTot = 0;
    for (var n = 1; n &lt;= nodeCount; n++) { gTxTot += (gossipTx[n]||0); gRxTot += (gossipRx[n]||0); }
    var expectedTargets = nodeCount - 1;
    var covSum = 0, covN = 0;
    for (var obs = 2; obs &lt;= nodeCount; obs++) {
        if (obs == MDC_ID) continue;
        if (cacheOf[obs]) {
            var known = 0;
            for (var k in cacheOf[obs]) if (cacheOf[obs][k] &gt; 0) known++;
            if (Object.keys(cacheOf[obs]).length &gt; 0) { covSum += known/expectedTargets; covN++; }
        }
    }
    var etaSc = (covN &gt; 0) ? (covSum * 100.0 / covN) : 0;
    var mdcKnown = 0;
    for (var kk in mdcCache) if (mdcCache[kk] &gt; 0) mdcKnown++;
    summary += "gossip_tx_total=" + gTxTot + "\n" +
               "gossip_rx_total=" + gRxTot + "\n" +
               "eta_sc_pct=" + etaSc.toFixed(2) + "\n" +
               "mdc_id=" + MDC_ID + "\n" +
               "mdc_joined_tsch=" + (mdcJoinedTsch ? 1 : 0) + "\n" +
               "mdc_contact_rx=" + mdcContactRx + "\n" +
               "mdc_first_contact_us=" + mdcFirstRxUs + "\n" +
               "mdc_known_targets=" + mdcKnown + "\n" +
               "mdc_coverage_pct=" + (mdcKnown * 100.0 / expectedTargets).toFixed(2) + "\n";
...
var lastReportUs = 0;
var joinedCount = 0;
var joinedSet = {};   // dedup: node ids that already emitted their first "Joined to"
...
function writeReport(header) {
    ...
    // -------- Gossip / cache convergence --------
    var gTxTot = 0, gRxTot = 0;
    for (var n = 1; n &lt;= nodeCount; n++) {
        gTxTot += (gossipTx[n] || 0);
        gRxTot += (gossipRx[n] || 0);
    }
    // Per-client cache coverage: mean fraction of network nodes known
    // (single-contact retrieval proxy: how much would MDC learn if it
    // contacted a typical client?)
    var expectedTargets = nodeCount - 1; // all clients (root skipped)
    var covSum = 0, covN = 0, covMin = 1e9, covMax = 0, covBestId = -1;
    for (var obs = 2; obs &lt;= nodeCount; obs++) {
        if (obs == MDC_ID) continue;
        var known = 0;
        if (cacheOf[obs]) {
        ...
        if (Object.keys(cacheOf[obs] || {}).length &gt; 0) {
            var frac = known / expectedTargets;
            covSum += frac; covN++;
            if (frac &lt; covMin) covMin = frac;
            if (frac &gt; covMax) { covMax = frac; covBestId = obs; }
        }
    }
    var etaSc = (covN &gt; 0) ? (covSum * 100.0 / covN) : 0;
    if (covN == 0) covMin = 0;

    log.log("Gossip         : tx = " + gTxTot + "   rx = " + gRxTot +
            "   observers = " + covN + "/" + (nodeCount - 1) + "\n");
    log.log("eta_sc         : mean = " + etaSc.toFixed(1) + " %" +
            "   min-client = " + (covMin*100).toFixed(1) + " %" +
            "   max-client = " + (covMax*100).toFixed(1) + " %  (single-contact coverage per typical client)\n");

    // -------- MDC section --------
    var mdcKnown = 0;
    for (var kk in mdcCache) if (mdcCache[kk] &gt; 0) mdcKnown++;
    var mdcCovPct = (mdcKnown * 100.0 / expectedTargets);
    var mdcFirstMs = (mdcFirstRxUs &gt; 0) ? (mdcFirstRxUs / 1000.0) : null;
    log.log("MDC            : joined_tsch = " + (mdcJoinedTsch ? "yes" : "no") +
            "   contacts = " + mdcContactRx +
            "   first_contact_ms = " + fmtOrNA(mdcFirstMs, 1) +
            "   known = " + mdcKnown + "/" + expectedTargets +
            " (" + mdcCovPct.toFixed(1) + " %)\n");
    log.log("=====\n");
}
...
while (true) {
    ...
    if (m) {
        ...
        var latRep = m[3] ? parseInt(m[3], 10) : -1;
        var key = srcId + "_" + seq;
        var aoiUs = -1;
    ...
    m = reJoin.exec(msg);
    if (m) {
        put("tsch_join.csv", t + "," + nid + ",0x" + m[1] + "," + m[2]);
        if (!joinedSet[nid]) { joinedSet[nid] = true; joinedCount++; }
        if (nid == MDC_ID &amp;&amp; !mdcJoinedTsch) {
            mdcJoinedTsch = true;
            put("mdc_events.csv", t + ",TSCH_JOIN,0x" + m[1] + " jp=" + m[2]);
        }
    }

    // -------- Gossip TX / RX --------
    if (reAdhocTx.test(msg)) {
        put("gossip_events.csv", t + "," + nid + ",TX,");
        if (!gossipTx[nid]) gossipTx[nid] = 0;
        gossipTx[nid]++;
    }
    if (reAdhocRx.test(msg)) {
        put("gossip_events.csv", t + "," + nid + ",RX,");
        if (!gossipRx[nid]) gossipRx[nid] = 0;
        gossipRx[nid]++;
        if (nid == MDC_ID) {
            mdcContactRx++;
            if (mdcFirstRxUs &lt; 0) {
                mdcFirstRxUs = t;
                put("mdc_events.csv", t + ",FIRST_CONTACT,");
            }
        }
    }

    // -------- Cache snapshot: "ADHOC_SYNC: &lt;observer&gt; [t1:s1] [t2:s2] ..." --------
    m = reAdhocSync.exec(msg);
    if (m) {
        var obs = parseInt(m[1], 10);
        var rest = m[2];
        if (!cacheOf[obs]) cacheOf[obs] = {};
        var entriesStr = "";
        var em;
        reAdhocEntry.lastIndex = 0;
        var known = 0;
        while ((em = reAdhocEntry.exec(rest)) !== null) {
            var tgt = parseInt(em[1], 10);
            var seq = parseInt(em[2], 10);
            if (seq &gt; 0) {
                cacheOf[obs][tgt] = seq;
                known++;
                if (!latestSeq[tgt] || seq &gt; latestSeq[tgt]) latestSeq[tgt] = seq;
                if (obs == MDC_ID) mdcCache[tgt] = seq;
            }
            entriesStr += tgt + ":" + seq + "|";
        }
        put("cache_snapshots.csv", t + "," + obs + "," + known + "," + entriesStr);
    }

    if (reMdcBoot.test(msg))    put("mdc_events.csv", t + ",BOOT,");
    if (reMdcStatus.exec(msg))  put("mdc_events.csv", t + ",STATUS,tick=" + (reMdcStatus.exec(msg) || [,""])[1]);
